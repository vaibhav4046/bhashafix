...
var RTL_SCRIPTS = /* @__PURE__ */ new Set(["Arab", "Hebr", "Thaa", "Nkoo", "Adlm", "Syrc"]);
var CHARACTER_BREAK_SCRIPTS = /* @__PURE__ */ new Set(["Hans", "Hant", "Jpan", "Kore"]);
var DICTIONARY_BREAK_SCRIPTS = /* @__PURE__ */ new Set(["Thai", "Laoo", "Khmr", "Mymr"]);
var TALL_GLYPH_SCRIPTS = /* @__PURE__ */ new Set(["Deva", "Taml", "Thai", "Ethi"]);
...
var REPRESENTATIVE_LOCALE_MATRIX = [
  "en-GB",
  "de-DE",
  "hi-IN",
  "ta-IN",
...
var RepairOperationSchema = external_exports.object({
  issueId: external_exports.string().min(1),
...
var RepairPlanSchema = external_exports.object({
  scanId: external_exports.string().min(1),
...
var VerificationResultSchema = external_exports.object({
  scanId: external_exports.string().min(1),
  ...
  consoleErrorDelta: external_exports.number().int().nullable(),
  accessibilityRegression: external_exports.boolean().nullable(),
  newBlockingIssues: external_exports.number().int().nonnegative(),
  diffWithinPolicy: external_exports.boolean().nullable(),
  /** Names of the fields above that were not measured and must not be read as evidence. */
  notMeasured: external_exports.array(external_exports.string()).default([])
});
...
var PathSchema = external_exports.array(external_exports.string().min(1)).min(1);
...
var PredicateSchema = external_exports.discriminatedUnion("predicate", [
  BaseSchema.extend({
...
// ../core/src/index.ts
var PredicateListSchema = external_exports.array(PredicateSchema).min(1);
var DEMO_FILES = {
...
var SENSITIVE_ATTRIBUTE = /(authorization|token|secret|password|api[-_]?key|session|cookie)/i;
...
function assertConfinedPath(projectRoot, file) {
  ...
  const target = path2.resolve(root, file);
...
async function assertNoSymlink(projectRoot, target) {
  ...
  const parentReal = await realpath(path2.dirname(target));
  const relative = path2.relative(rootReal, parentReal);
  if (relative.startsWith("..") || path2.isAbsolute(relative)) {
...
function lcsEdits(before, after) {
  ...
  for (let row2 = rows - 1; row2 >= 0; row2--) {
    for (let column2 = columns - 1; column2 >= 0; column2--) {
      lengths[row2][column2] = diffLineKey(before[row2]) === diffLineKey(after[column2]) ? lengths[row2 + 1][column2 + 1] + 1 : Math.max(lengths[row2 + 1][column2], lengths[row2][column2 + 1]);
    }
  }
  const edits = [];
  let row = 0;
...
function diffEdits(before, after) {
  let head = 0;
  while (head < before.length && head < after.length && diffLineKey(before[head]) === diffLineKey(after[head])) {
    head += 1;
  }
  let tail = 0;
  while (tail < before.length - head && tail < after.length - head && diffLineKey(before[before.length - 1 - tail]) === diffLineKey(after[after.length - 1 - tail])) {
    tail += 1;
  }
  const beforeMiddle = before.slice(head, before.length - tail);
  const afterMiddle = after.slice(head, after.length - tail);
  const middle = beforeMiddle.length * afterMiddle.length > MAX_LCS_CELLS ? [
    ...beforeMiddle.map((line) => ({ kind: "delete", line })),
    ...afterMiddle.map((line) => ({ kind: "insert", line }))
  ] : lcsEdits(beforeMiddle, afterMiddle);
  return [
    ...before.slice(0, head).map((line) => ({ kind: "equal", line })),
    ...middle,
    ...before.slice(before.length - tail).map((line) => ({ kind: "equal", line }))
  ];
}
...
function buildUnifiedDiff(file, beforeText, afterText) {
  ...
  const hunks = ranges.map((range) => {
    ...
    const beforeStart = beforeCount ? beforeLineNumbers[range.start] : beforeLineNumbers[range.start] - 1;
    const afterStart = afterCount ? afterLineNumbers[range.start] : afterLineNumbers[range.start] - 1;
    return [
...
async function buildOperationsDiff(projectRoot, operations) {
...
async function prepareRepair(projectRoot, scan) {
  ...
  for (const predicate of predicates) {
    ...
    const json = JSON.parse(await readFile2(target, "utf8"));
    operations.push({
      issueId: predicate.issueId,
...
var BROWSER_ONLY_FIELDS = [
  "consoleErrorDelta",
  "accessibilityRegression"
];
var DEFAULT_MAX_CHANGED_LINES = 200;
function countChangedDiffLines(unifiedDiff) {
  return unifiedDiff.split("\n").filter(
    (line) => line.startsWith("+") && !line.startsWith("+++") || line.startsWith("-") && !line.startsWith("---")
  ).length;
}
function evaluateDiffPolicy(plan, policy = {}) {
  ...
  const changedFiles = new Set(plan.operations.map((operation) => operation.file)).size;
  ...
  return {
    ...
    changedFiles,
    maxChangedFiles,
    withinPolicy: changedLines <= maxChangedLines && changedFiles <= maxChangedFiles
  };
}
async function verifyRepair(projectRoot, baseline, options = {}) {
  ...
  const diffPolicy = options.repairPlan ? evaluateDiffPolicy(options.repairPlan, options.diffPolicy) : null;
  const notMeasured = [
    ...BROWSER_ONLY_FIELDS,
  ...
  const result = VerificationResultSchema.parse({
    ...
    status: finalBlocking === 0 && sourceLocaleRegression === "PASS" && diffPolicy?.withinPolicy !== false ? "verified" : "verification failed",
    baselineBlocking,
    finalBlocking,
