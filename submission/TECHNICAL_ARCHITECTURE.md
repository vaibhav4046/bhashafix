...
The hosted product has two deliberately separate paths. `POST /api/scan/browser`
runs bounded real Chromium for one route, up to three locales and one viewport,
returns screenshots and measured DOM/axe evidence, and persists nothing.
`POST /api/scan` is the static HTTP preflight. Full route x locale x viewport
matrices, authenticated coverage, durable artifacts and repository repair run
...
The recorded MCP 10-to-0 repair sequence drives this AtlasPay fixture through
the built STDIO server. It proves transport, schemas, explicit-ID guardrails,
dry-run/application and verification orchestration; it is not evidence of
arbitrary-project MCP repair.
